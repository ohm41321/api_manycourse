...
<body>
  <div class="container">
      <h1>{{ $cat->where('categories_id', 5)->first()->categories_name }} Programps</h1>
      @foreach ($course->where('categories_id', 5) as $sport)
      <div class="course-details">
          <h2 class="course-name">{{ $sport->course_name }}</h2>
          <p class="description">{{ $sport->descriptions }}</p>
          <p class="instructor">Instructor: <span>{{ $instructor[$sport->instructors_code] }}</span></p>
      </div>
      @endforeach
  </div>
</body>
