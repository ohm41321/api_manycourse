...
<body>
  <div class="container">
      <h1>{{ $cat->where('categories_id', 2)->first()->categories_name }} Programps</h1>
      @foreach ($course->where('categories_id', 2) as $sci)
      <div class="course-details">
          <h2 class="course-name">{{ $sci->course_name }}</h2>
          <p class="description">{{ $sci->descriptions }}</p>
          <p class="instructor">Instructor: <span>{{ $instructor[$sci->instructors_code] }}</span></p>
      </div>
      @endforeach
  </div>
</body>
